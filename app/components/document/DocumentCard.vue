...
<template>
  ...
  >
    ...
    <div v-else class="relative flex items-start gap-4">
      ...
      <UDropdownMenu size="sm" :items="documentActions" :content="{ align: 'end' }" class="absolute top-0 right-0">
  ...
  <UCard v-else :class="{ 'ring-neutral ring-2 ring-offset-2': isEditing }">
    <!-- Edit Mode -->
    <div v-if="isEditing && editingDocument" class="space-y-3">
      <div class="flex items-center gap-3">
        <div class="bg-primary/10 text-primary flex size-10 items-center justify-center rounded-full">
          <UIcon :name="getDocumentIcon(editingDocument.category)" class="text-xl" />
        </div>
        <div>
    ...
    <div v-else class="relative">
      <div class="flex items-start justify-between">
        <div class="flex min-w-0 items-center gap-3">
          <div class="bg-primary/10 text-primary flex size-10 items-center justify-center rounded-full">
            <UIcon :name="getDocumentIcon(patientDocument.category)" size="xl" class="text-iiixl" />
          </div>
          <div class="min-w-0">
            ...
            <!-- Description -->
            <h3 class="text-default truncate text-sm font-bold">
              {{ patientDocument.description || patientDocument.originalFileName }}
            </h3>
        ...
        <UDropdownMenu size="sm" :items="documentActions" :content="{ align: 'end' }">
          <UButton icon="i-hugeicons-more-vertical" variant="ghost" color="neutral" size="sm" square />
        </UDropdownMenu>
      </div>
      ...
      <div class="mt-4 space-y-2">
        <!-- Metadata Grid -->
        <div class="text-muted grid grid-cols-2 gap-x-4 gap-y-2 pt-2 text-xs">
