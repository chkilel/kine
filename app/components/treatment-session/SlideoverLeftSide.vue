<script setup lang="ts">
  ...
  const sessionStatusConfig = computed(() => {
    const status = appointment?.treatmentSession?.status
  ...
  const painLevelBefore = computed(() => appointment?.treatmentSession?.painLevelBefore ?? null)
</script>
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      <div class="flex flex-col items-center p-6">
        ...
        <div class="text-center">
          ...
          <div v-if="sessionStatusConfig" class="mt-3">
            <UBadge
              :color="sessionStatusConfig.color"
              :label="sessionStatusConfig.label"
              :icon="sessionStatusConfig.icon"
              size="md"
              variant="subtle"
            />
          </div>
