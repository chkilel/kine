<script setup lang="ts">
  ...
  // ─── Zod schema (reactive to credit balance) ────────────────────────────────
  const formSchema = computed(() => paymentFormSchemaFactory(patientCreditBalance.value || 0))
...
<template>
  <AppCard variant="outline" title="Enregistrer un paiement">
    <UForm :state="state" :schema="formSchema" @submit="onSubmit" class="space-y-6">
