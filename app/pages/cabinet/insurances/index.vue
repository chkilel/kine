<script setup lang="ts">
  import { LazyInsuranceCompanySlideover, LazyAppModalConfirm } from '#components'

  const overlay = useOverlay()
  const formOverlay = overlay.create(LazyInsuranceCompanySlideover)
...
<template>
  <AppDashboardPage id="settings-insurance-companies" title="Assurances">
    ...
    >
      ...
      <div v-else-if="error" class="text-error p-8 text-center">
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-3">
              <h3 class="truncate font-medium">{{ company.name }}</h3>
            ...
            <p class="mt-1 text-sm">Code: {{ company.code }}</p>
            <div class="mt-2 flex items-center gap-4 text-sm">
              ...
              <span class="">
                ...
                Prix: {{ formatCurrency(company.sessionPriceCents) }}
              </span>
              <span class="">
                ...
                {{
                  company.coPayRule === 'fixed'
                    ? `${formatCurrency(company.coPayAmountCents!)}`
