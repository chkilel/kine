...
// ─── Composables ─────────────────────────────────────────────
const { openRecordPayment, openPaymentHistory, openAddDeposit, openRefundBalance } =
    useBillingSlideover()
...
const appointments = computed(
    () => appointmentsData.value?.pages.flatMap((page) => page.data) ?? []
)
...
const planFilteredAppointments = computed(() =>
    appointments.value.filter((s) => {
        ...
        if (
            ...
            selectedPlan.value !== 'no-plan' &&
            s.treatmentPlanId !== selectedPlan.value
        )
            return false
        return true
    })
            ...
            <div class="bg-muted flex items-center gap-0.5 rounded-xl p-1">
                ...
                <UPopover v-model:open="planPopoverOpen">
                    <UButton
                        ...
                        class="rounded-lg"
                        :trailing-icon="
                            activePlanSegment === 'by-plan'
                                ? 'i-lucide-check'
                                : 'i-lucide-chevron-down'
                        "
                    >
                        {{
                            activePlanSegment === 'by-plan' && selectedPlanName
                                ? selectedPlanName
                                : 'Par plan'
                        }}
                    </UButton>
                    <template #content>
                        <div class="flex min-w-56 flex-col gap-1 p-1">
                            ...
                            >
                                ...
                                {{ plan.name }}
                                <UBadge size="xs" variant="subtle" class="ml-auto">{{
                                    plan.count
                                }}</UBadge>
                            </UButton>
                        </div>
        ...
        <div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-3">
            ...
            <aside class="space-y-4">
                ...
                <PaymentBillingFinancialSummaryCard
                    :sessions="planFilteredAppointments"
                    :patient-id="patientId"
                />
            </aside>
        </div>
