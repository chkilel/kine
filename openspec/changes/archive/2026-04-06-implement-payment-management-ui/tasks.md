...
- [x] 1.1 Review Stitch reference HTML files in `references/stitch-screens/` for layout structure (especially `screen7-facturation-filter-updated.html`)
...
- [x] 1.4 Review Nuxt UI components: `USlideover`, `UModal`, `UButton`, `UBadge`, `UProgress`, `UAlert`, `UCheckbox`, `UPopover`

## 2. Main Billing Page

- [x] 2.1 Rewrite `app/pages/patients/[id]/facturation.vue` with 3-column grid layout
- [x] 2.2 Add plan segment filter: 3-button segment (`Toutes (N)` | `Sans plan (N)` | `Par plan ▾`) with `UPopover` dropdown for plan selection
- [x] 2.3 Add status filter: pill button group (`Tous` | `Non facturé` | `Partiellement` | `Payé`)
- [x] 2.4 Add "Séances à facturer" section with session cards (static mock data, 3 statuses)
- [x] 2.5 Add payment history preview section with recent payments list + "Voir tout l'historique" link
- [x] 2.6 Place "Enregistrer un paiement" primary button in right sidebar (above balance card)
...
- [x] 11.2 Run `pnpm lint` and fix any issues (no lint script configured)
- [x] 11.3 Run `pnpm typecheck` and fix any type errors (all errors pre-existing, none in billing code)
