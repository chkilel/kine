...
The system SHALL support updating the price and extending the duration on the appointment record.

#### Scenario: Update appointment price

- **GIVEN** an appointment exists with status "finished"
- **WHEN** PATCH /api/appointments/[id]/price is called with body { priceCents: 6000 }
- **THEN** priceCents is updated to 6000
- **AND** HTTP response is 200 OK
...
The system SHALL enrich the GET /api/appointments endpoint with per-appointment payment status when `includePaymentStatus=true` query parameter is provided. Each appointment includes `paidCents` and `paymentStatus`.

#### Scenario: Enrich appointments with payment status

- **GIVEN** a patient has appointments with payment status: apt-1 paid (15000/15000), apt-2 unpaid (0/15000)
...
The system SHALL store all scheduling, clinical, timer, billing, and locking data in a single `appointments` table. No `treatment_sessions` table exists.
