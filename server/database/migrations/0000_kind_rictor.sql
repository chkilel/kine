...
CREATE TABLE `payment_session_items` (
	...
	CONSTRAINT `fk_payment_session_items_paymentId_payments_id_fk` FOREIGN KEY (`paymentId`) REFERENCES `payments`(`id`) ON DELETE CASCADE
);
--> statement-breakpoint
...
CREATE TABLE `treatment_sessions` (
	...
	`status` text DEFAULT 'pre_session' NOT NULL,
	...
	`createdAt` integer NOT NULL,
	`updatedAt` integer NOT NULL,
	CONSTRAINT `fk_treatment_sessions_organizationId_organizations_id_fk` FOREIGN KEY (`organizationId`) REFERENCES `organizations`(`id`) ON DELETE CASCADE,
	CONSTRAINT `fk_treatment_sessions_appointmentId_appointments_id_fk` FOREIGN KEY (`appointmentId`) REFERENCES `appointments`(`id`) ON DELETE CASCADE,
	CONSTRAINT `fk_treatment_sessions_patientId_patients_id_fk` FOREIGN KEY (`patientId`) REFERENCES `patients`(`id`) ON DELETE CASCADE,
	CONSTRAINT `fk_treatment_sessions_treatmentPlanId_treatment_plans_id_fk` FOREIGN KEY (`treatmentPlanId`) REFERENCES `treatment_plans`(`id`) ON DELETE SET NULL,
	CONSTRAINT `fk_treatment_sessions_therapistId_users_id_fk` FOREIGN KEY (`therapistId`) REFERENCES `users`(`id`)
);
--> statement-breakpoint
...
CREATE INDEX `idx_treatment_plans_org_active_therapist_status` ON `treatment_plans` (`organizationId`,`therapistId`,`status`);--> statement-breakpoint
CREATE INDEX `idx_treatment_sessions_patient_date` ON `treatment_sessions` (`patientId`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_treatment_sessions_appointment` ON `treatment_sessions` (`appointmentId`);--> statement-breakpoint
CREATE UNIQUE INDEX `ux_treatment_sessions_appointment` ON `treatment_sessions` (`appointmentId`);