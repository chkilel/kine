...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const body = await readValidatedBody(event, updateTagsActionSchema.parse)
    ...
    const tagsValue = body.tags && body.tags.length > 0 ? JSON.stringify(body.tags) : null
    ...
    return successResponse(updated, 'Tags mis à jour')
  } catch (error: unknown) {
    handleApiError(error, 'Failed to update treatment session tags')
  }
})
